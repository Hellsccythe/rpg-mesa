...
PersonagensRouter.get("/", async (req, res) => {
    try {
        ...
        const resultado = await personagensController.listarMeus(req.query, token);
        res.status(200).json(resultado);
    }
    catch (error) {
...
PersonagensRouter.post("/admin/solicitacoes/:characterId/revisar", async (req, res) => {
    try {
        const token = getBearerToken(req.headers.authorization);
        const resultado = await personagensController.revisarSolicitacao(req.params.characterId, req.body, token);
        res.status(200).json(resultado);
    }
    catch (error) {
        const status = error?.message?.includes("autenticado") || error?.message?.includes("restrito") ? 401 : 400;
...
PersonagensRouter.post("/admin/personagens/:characterId/notas", async (req, res) => {
    try {
        const token = getBearerToken(req.headers.authorization);
        const resultado = await personagensController.adicionarNotaAventura(req.params.characterId, req.body, token);
        res.status(200).json(resultado);
    }
    catch (error) {
        const status = error?.message?.includes("autenticado") || error?.message?.includes("restrito") ? 401 : 400;
...
PersonagensRouter.get("/admin/:characterId", async (req, res) => {
    try {
        ...
        const status = error?.message?.includes("autenticado") || error?.message?.includes("restrito") ? 401 : 404;
...
PersonagensRouter.patch("/:characterId/solicitacao", async (req, res) => {
    try {
        const token = getBearerToken(req.headers.authorization);
        const resultado = await personagensController.solicitarAlteracao(req.params.characterId, req.body, token);
        res.status(200).json(resultado);
    }
    catch (error) {
