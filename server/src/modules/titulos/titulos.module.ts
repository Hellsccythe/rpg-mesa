...
TitulosRouter.patch("/admin/:id", async (req, res) => {
  try {
    ...
    const { name, tier, description, skillIds, bonuses, is_hidden, linked_hidden_class } = req.body;
    const resultado = await titulosService.editar(id, { name, tier, description, skillIds, bonuses, is_hidden, linked_hidden_class } as any, token);
    res.status(200).json(resultado);
  } catch (error: any) {
